...
def_list = []
var_list = []
a = open("4-20-Python-IO-81-Rybnikov.txt", 'r')
input = a.read()
lexer = Lexer()
...
def match(a):
    ...
    if lookahead.tag == a:
        ...
        print(f"SyntaxError {str(lexer.line)} line, {str(lexer.error)} index")
        os.system("PAUSE")
        sys.exit(1)
    ...
    if lookahead.tag == PLUS:
        ...
        while lookahead.tag == WHILE:
            ...
            if lookahead.value in var_list:
                ...
                print(f"Unknown function in {str(lexer.line)} line, {str(lexer.error)} index")
                os.system("PAUSE")
                sys.exit(1)
